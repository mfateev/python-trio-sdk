...
import temporalio_trio.activity
import temporalio_trio.workflow
from temporalio_trio.workflow import VersioningIntent
...
class ActivityOutboundInterceptor:
    ...
    def info(self) -> temporalio_trio.activity.Info:
        """Called for every :py:func:`temporalio_trio.activity.info` call."""
        return self.next.info()

    def heartbeat(self, *details: Any) -> None:
        """Called for every :py:func:`temporalio_trio.activity.heartbeat` call."""
...
class StartChildWorkflowInput:
    ...
    id: str
    task_queue: Optional[str]
    cancellation_type: temporalio_trio.workflow.ChildWorkflowCancellationType
    parent_close_policy: temporalio_trio.workflow.ParentClosePolicy
...
class WorkflowOutboundInterceptor:
    ...
    def continue_as_new(self, input: ContinueAsNewInput) -> NoReturn:
        """Called for every :py:func:`temporalio_trio.workflow.continue_as_new` call."""
        self.next.continue_as_new(input)

    def info(self) -> temporalio_trio.workflow.Info:
        """Called for every :py:func:`temporalio_trio.workflow.info` call."""
        return self.next.info()

    async def signal_child_workflow(self, input: SignalChildWorkflowInput) -> None:
        """Called for every
        :py:meth:`temporalio_trio.workflow.ChildWorkflowHandle.signal` call.
        """
    ...
    async def signal_external_workflow(
        ...
        """Called for every
        :py:meth:`temporalio_trio.workflow.ExternalWorkflowHandle.signal` call.
        """
        return await self.next.signal_external_workflow(input)

    def start_activity(
        self, input: StartActivityInput
    ) -> temporalio_trio.workflow.ActivityHandle:
        """Called for every :py:func:`temporalio_trio.workflow.start_activity` and
        :py:func:`temporalio_trio.workflow.execute_activity` call.
        """
        return self.next.start_activity(input)

    async def start_child_workflow(
        self, input: StartChildWorkflowInput
    ) -> temporalio_trio.workflow.ChildWorkflowHandle[Any, Any]:
        """Called for every :py:func:`temporalio_trio.workflow.start_child_workflow`
        and :py:func:`temporalio_trio.workflow.execute_child_workflow` call.
        """
    ...
    def start_local_activity(
        ...
        """Called for every :py:func:`temporalio_trio.workflow.start_local_activity`
        and :py:func:`temporalio_trio.workflow.execute_local_activity` call.
        """
